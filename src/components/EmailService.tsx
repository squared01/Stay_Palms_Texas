...
import { generateReminderEmail } from '../utils/emailService';
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-800">Email Reminders</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
          <div className="flex items-center">
            ...
            <span className="font-semibold text-yellow-800">Pending Emails</span>
          </div>
          <p className="text-2xl font-bold text-yellow-600 mt-1">
            {pendingReminders.length}
          </p>
          <p className="text-xs text-yellow-700 mt-1">Reminders & Confirmations</p>
        </div>
        
        <div className="bg-green-50 p-4 rounded-lg border border-green-200">
          <div className="flex items-center">
            <CheckCircle className="w-5 h-5 text-green-600 mr-2" />
            <span className="font-semibold text-green-800">Emails Sent</span>
          </div>
          <p className="text-2xl font-bold text-green-600 mt-1">
            {reservations.filter(r => r.reminderSent).length}
          </p>
          <p className="text-xs text-green-700 mt-1">All email notifications</p>
        </div>
        
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <div className="flex items-center">
            <AlertCircle className="w-5 h-5 text-blue-600 mr-2" />
            <span className="font-semibold text-blue-800">Total Reservations</span>
          </div>
      ...
      {pendingReminders.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          ...
          <p>No pending email notifications at this time</p>
        </div>
      )}
    </div>
  );
